    ...
    def run(self):
        ...
        with JsonLogger(log_path) as json_logger:
            for local_epoch_idx in range(cfg.training.num_epochs):
                ...
                if ((self.epoch + 1) % cfg.training.checkpoint_every) == 0:
                    # === 使用 hydra.run.dir 下的 checkpoint 目录 ===
                    ckpt_dir = os.path.join(self.output_dir, "checkpoints")
                    os.makedirs(ckpt_dir, exist_ok=True)

                    ckpt_path = os.path.join(ckpt_dir, f"epoch_{self.epoch + 1}.ckpt")
...
class BatchSampler:
    ...
    def __init__(
        ...
        # ✅ 计算 remainder 并决定是否丢弃
        remainder = data_size % batch_size
        self.discard = remainder if drop_last and remainder > 0 else 0
        self.num_batch = (data_size - self.discard) // batch_size
...
# ✅ 1. 提前定义 collate
def collate(samples):
    # 检查是否已经是整批数据（Dataset直接返回的）
    if isinstance(samples, dict):
        # 检查第一个值是否已经是批次格式（有batch维度）
    ...
    for k in keys:
        # numpy.stack 比 torch.stack 快很多，并且数据还在 numpy 占用的内存中
        np_batch = np.stack([s[k] for s in samples], axis=0)

        # torch.as_tensor 不会 copy numpy 内存（共享内存）
        batch[k] = torch.as_tensor(np_batch, device='cpu')

    return batch


# ✅ 2. create_dataloader 只保留一次 DataLoader 构建
def create_dataloader(
    ...
    *,
    ...
    dataloader = DataLoader(
        dataset,
        batch_sampler=batch_sampler,         # ✅ 使用自定义批采样器
        num_workers=num_workers,
        pin_memory=pin_memory,
        persistent_workers=persistent_workers,
        collate_fn=collate,                  # ✅ 使用上面定义的 collate
    )
