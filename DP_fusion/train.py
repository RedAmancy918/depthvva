...
def main(cfg: OmegaConf):
    ...
    # 同时设置 head_camera_depth 的shape（如果存在）
    if "head_camera_depth" in cfg.task.shape_meta.obs:
        cfg.task.shape_meta.obs.head_camera_depth.shape = [
            3,
            head_camera_cfg["h"],
            head_camera_cfg["w"],
        ]
    ...
    # 再次设置 head_camera_depth 的shape（如果存在）
    if "head_camera_depth" in cfg.task.shape_meta.obs:
        cfg.task.shape_meta.obs.head_camera_depth.shape = [
            3,
            head_camera_cfg["h"],
