...
class RobotImageDataset(BaseImageDataset):
    ...
    def __init__(
        ...
        self.replay_buffer = ReplayBuffer.copy_from_path(
            zarr_path,
            # keys=['head_camera', 'front_camera', 'left_camera', 'right_camera', 'head_camera_depth', 'state', 'action'],
            keys=["head_camera", "head_camera_depth", "state", "action"],
        )
    ...
    def get_normalizer(self, mode="limits", **kwargs):
        ...
        normalizer["head_camera_depth"] = get_image_range_normalizer()
    ...
    #     # 深度图已经是 (N, 1, H, W) 格式，需要扩展到3通道
    #     depth = sample["head_camera_depth"] / 255.0  # 归一化
    #     if depth.shape[1] == 1:  # (N, 1, H, W)
    #         depth = np.repeat(depth, 3, axis=1)  # -> (N, 3, H, W)
    ...
    #             "head_camera_depth": depth,  # T, 3, H, W (扩展后)
    ...
    # ✅ 兼容 Python int + NumPy int + 0维 torch 张量
    ...
    def postprocess(self, samples, device):
        ...
        # 处理深度图：单通道扩展到3通道
        depth = samples["head_camera_depth"].to(device, non_blocking=True) / 255.0
        if depth.shape[2] == 1:  # (B, T, 1, H, W)
            depth = depth.repeat(1, 1, 3, 1, 1)  # -> (B, T, 3, H, W)
        ...
        return {
            "obs": {
                ...
                "head_camera_depth": depth,  # B, T, 3, H, W
